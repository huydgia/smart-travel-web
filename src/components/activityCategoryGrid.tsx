...
export const ActivityCategoryGrid = () => {
  return (
    <section className="bg-gray-100 dark:bg-zinc-800 px-4 md:px-10 py-10 space-y-4">
      ...
      <p className="text-gray-600 dark:text-gray-300">Chúng tôi có mọi trải nghiệm bạn yêu thích</p>

      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {activities.map((item, index) => (
          <div
            key={index}
            className="relative h-44 rounded-xl overflow-hidden group shadow-md"
          >
            <Image
              src={item.image}
              alt={item.title}
