      ...
      <Container>
        <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-8">
          ...
          {footerLinks.map((section, i) => (
            <div key={i} className="hidden md:block">
              <h4 className="font-semibold text-zinc-700 dark:text-white mb-2">
                {t(section.title)}
              </h4>
        ...
        <div className="max-w-6xl mx-auto mt-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-center sm:text-left">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-semibold text-zinc-600 dark:text-zinc-200 mr-4">
              Ngôn ngữ
            </h4>
            <LanguageSwitcher />
          </div>

          <div className="text-xs text-gray-500 dark:text-gray-400">Quốc gia: Việt Nam</div>
        </div>
        {/* Copyright */}
        <div className="text-[11px] mt-8 pt-4 sm:text-xs text-center text-xs text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-zinc-700">
          ...
          © {new Date().getFullYear()} Huy Halu. Được phát triển với cảm hứng từ Tripadvisor.
        </div>
      </Container>
    </footer>
  );
